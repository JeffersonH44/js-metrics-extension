...
var VERSION_INFO = { version: "2.0.5", build: 1525518909503 };
  ...
  _updateDecorations: function _updateDecorations(duration) {
    ...
    if (showxAxisGroups) {
      ...
      xAxisGroups.each(function (d) {
        ...
        var text = view.select("text").text(_this.translator(d.label));

        var textHeight = text.node().getBBox().height;

        text.transition().duration(duration || 0).attr("y", textHeight).attr("x", _this.xScale(d.min || d.min === 0 ? d.min : _this.model.marker.axis_x.domainMin) / 2 + _this.xScale(d.max || d.max === 0 ? d.max : _this.model.marker.axis_x.domainMax) / 2);

        var showLineMax = d.max || d.max === 0;
        view.select("line.vzb-bc-x-axis-group-line").classed("vzb-invisible", !showLineMax);

        if (showLineMax) {
          view.select("line.vzb-bc-x-axis-group-line").transition().duration(duration || 0).attr("y1", 0).attr("y2", textHeight).attr("x1", _this.xScale(d.max)).attr("x2", _this.xScale(d.max));
        }
      });
    }
